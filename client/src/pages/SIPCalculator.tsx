...
import { sipContent } from "@/content/calculators/sip.content";
import * as calc from "@/features/calculators/utils";

type Currency = 'INR' | 'USD' | 'AED';

export default function SIPCalculator() {
  const calculator = getCalculatorBySlug('sip')!;
  const prevCalc = getPreviousCalculator('sip');
  const nextCalc = getNextCalculator('sip');
  ...
  const returnsPercent = ((sipResults.returns / sipResults.futureValue) * 100).toFixed(1);
  const investedPercent = ((sipResults.invested / sipResults.futureValue) * 100).toFixed(1);
